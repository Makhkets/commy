...
| 13 | **Наборы правил, исключение E-2.** Загрузка по кнопке + передать `ruleSetDirectory` в сборщик. Пока не сделано, правила `geosite:` выбрасываются (теперь хотя бы вслух) | крупно | `routing_screen.dart`, `selector_config_generator.dart` |
