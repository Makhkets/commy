...
Ещё два, `openVpnSettings` и `setStartOnBoot`, к туннелю не относятся и
описаны отдельно в конце раздела — они говорят с системой и ничего про
туннель не знают.
...
- [ ] семь туннельных методов плюс `openVpnSettings` и `setStartOnBoot`,
      каждый **всегда**
