...
Ровно семь команд туннелю. Больше туннельных методов на этом канале нет.

Ещё шесть к туннелю не относятся и описаны отдельно в конце раздела:
`openVpnSettings`, `setStartOnBoot`, `installedApps` и `deviceInfo` говорят с
системой, `probeOutbounds` и `ping` меряют серверы пользователя, туннель не
трогая.
...
Открывает системный экран настроек VPN. Аргумента нет, результата нет.

**Не команда туннелю** и потому не входит в семёрку. Метод существует по одной
причине: единственный kill switch на Android, который действительно держит, —
системный «Постоянный VPN» вместе с «Блокировать соединения без VPN». Само
приложение такой гарантии дать не может: у убитого процесса нечем блокировать.
Поэтому строка в настройках называет механизм, говорит, что он принадлежит
системе, и предлагает его открыть.

Если экрана нет — на части прошивок его прячут, на Android TV его не бывает —
Kotlin откатывается на общий экран настроек. Dart-обёртка `SystemSettings`
возвращает `false` вместо исключения: строка настроек не имеет права уронить
приложение.

### `setStartOnBoot(enabled: Boolean) → null`

Включает или выключает `BootReceiver` через
`PackageManager.setComponentEnabledSetting`. Аргумент — голый `Boolean`, не
JSON. Результата нет.

**Не команда туннелю.** Переключатель «Автозапуск при загрузке» хранится на
Dart-стороне, а компонент, который его исполняет, объявлен в манифесте с
`enabled="false"`. Этот метод — единственное, что связывает одно с другим: без
него переключатель пишется и ничего не меняет. Dart зовёт его после успешной
записи настройки и один раз при запуске, чтобы после переустановки или
восстановления из бэкапа компонент сошёлся с настройкой.

Что receiver умеет — только уведомление с предложением открыть приложение;
почему не больше — KDoc в `BootReceiver.kt` (правило R2).

Dart-обёртка `SystemSettings.setStartOnBoot` возвращает `false` там, где
receiver'а нет — на десктопах, — и не бросает.

### `installedApps() → String`

Возвращает JSON-массив строкой: `[{"package": …, "label": …, "isSystem": …}]`.
Аргумента нет.

| Поле | Обяз. | Описание |
|---|---|---|
| `package` | да | application id, например `org.mozilla.firefox` |
| `label` | да | как приложение называется в лаунчере, на языке устройства |
| `isSystem` | нет | пришло ли приложение с образом системы; по умолчанию `false` |

**Не команда туннелю.** Режим и список пакетов для per-app маршрутизации
живут на Dart-стороне и попадают в конфиг как `include_package` /
`exclude_package`; единственное, чего Dart знать не может, — какие приложения
на устройстве вообще есть.

**Только запускаемые приложения.** Kotlin спрашивает
`queryIntentActivities(MAIN + LAUNCHER)` и опирается на блок `<queries>` в
манифесте, а **не** на `QUERY_ALL_PACKAGES`: широкое разрешение Play считает
чувствительным, и полный список пакетов пользователя приватного инструмента —
не та возможность, которая нам нужна. Цена честная: фоновое приложение без
иконки в этот список не попадёт.

**Без иконок.** Несколько сотен битмапов через method channel ради списка,
который пролистывают, — плохой размен. Строка и имя пакета.

Свой собственный пакет Kotlin выбрасывает из ответа: он и так всегда вне
туннеля (`InboundSectionBuilder`), и предлагать его как выбор — предлагать
выбор, которого нет.

Dart-обёртка `SystemSettings.installedApps` возвращает **пустой список** там,
где маршрутизации по приложениям нет — на всех десктопах, — и не бросает.
Сортировку делает она же: по названию без учёта регистра, системные в конце.

### `probeOutbounds(args: String) → String`

Метод GET раздела «Пинг» ([ADR-0015](../../../docs/adr/0015-ping-methods.md)).

```json
{"config": "{\"log\":{\"disabled\":true},\"outbounds\":[…]}",
 "url": "http://cp.cloudflare.com/generate_204", "timeoutMs": 8000}
```

```json
{"node-7f3c1a": 212, "node-9b21": 0}
```

`config` — **строка** с конфигурацией sing-box (`SingBoxConfigBuilder.buildProbe`:
только исходящие узлов, прямой резолвер, лог выключен), `url` — адрес проверки,
`timeoutMs` — сколько ждать каждый сервер. Ответ — задержка каждого исходящего
в миллисекундах, `0` — «не ответил».

**Не команда туннелю.** Kotlin вызывает `Mobile.urlTestOutbounds` из
`core/mobile`: отдельный экземпляр ядра поднимает исходящие, делает через
каждый GET и закрывается. Работает при снятом туннеле и при поднятом, туннель
не трогает. Ошибка (`config_invalid`) — только если ядро отвергло
конфигурацию целиком; сервер, который не ответил, — это `0`, а не ошибка.

### `ping(args: String) → String`

Метод ICMP раздела «Пинг».
...
```json
{"delayMs": 38}      // эхо вернулось
{"delayMs": null}    // не вернулось — многие серверы на ICMP не отвечают
```

Kotlin разрешает имя до старта часов и шлёт одно эхо с непривилегированного
«ping-сокета» (`SOCK_DGRAM` + `IPPROTO_ICMP`), который Android открывает
любому приложению. `dart:io` ICMP не умеет. Dart-обёртка
`SystemSettings.ping` не бросает: платформа без метода — это `null`.
...
- [ ] семь туннельных методов плюс `openVpnSettings`, `setStartOnBoot` и
      `installedApps`, каждый **всегда**
      отвечает `success` или `error`, без тихих веток;
