...
Что есть: шесть пакетов, 777 тестов плюс 99 golden (только на Linux), чистый
...
**Последний прогресс:** 2026-09-11 — закрыт **#14**: группа «Авто» (urltest)
дошла от сборщика до экрана. `AppSettings.autoSelect`, строка «Авто» над
списком, узел выбора ядра виден по `proxies()`. Включение на живом туннеле
пересобирает документ **только** если группы в нём нет, а нажатие на сервер
выключает «Авто» без рестарта. Заодно `FakeCoreClient` читает группы из
документа — до этого на десктопе `select` любого настоящего узла падал.
Закрыт и **#17**: `CoreClient.dispose()` в порту, core client и HTTP-клиент
закрываются провайдером, БД и логгер `main()` отдаёт графу через
`ownedOverride` (у `overrideWithValue` тело провайдера не выполняется, и
`onDispose` регистрировать негде).
Приложение: 127 тестов, всего 810 + 99 golden; `commy_ui` не тронут.
**Не сделано и важно:** чек-лист утечек (R6) после #13 — требует телефона;
Kotlin-половина #12 здесь не компилировалась; «Авто» ни разу не видело живого
туннеля. Следующему: хвост очереди — **#15**, **#18**, **#16**, **#19**;
детали — в docs/17-agent-handoff.md, «Сессия 6».
