...
Выпущено **`v0.1.0-alpha.5`** (2026-09-23): на странице релиза один APK на
51,4 МБ (бюджет 56) и `SHA256SUMS.txt`. Он, как и alpha.4, **не подписан
релизным ключом** (секретов подписи в CI нет — решение владельца, §7.5), поэтому
поверх alpha.4 не ставится: сначала удалить. UX-проход, сессии 15 и 16 — в `main`.

Что есть: шесть пакетов, **1430** Dart-тестов (из них 111 golden, только на
Linux) и 46 Go-тестов, чистый анализ, ядро sing-box под три ABI **с транспортом
XHTTP**, Android-туннель (`VpnService`, `PlatformInterface` 15/15,
`BootReceiver`, плитка, Doze), все экраны приложения, deep links.

**Последний прогресс:** 2026-09-23, сессии 15–16 — **лёгкий эмулятор, стенд с
живыми узлами, полный прогон Android и выпуск alpha.5.** Эмулятор на этой
слабой машине: `scripts/emulator.sh`; стенд — девять узлов всех протоколов,
панель как Remnawave с лимитом HWID, зонд трафика: `scripts/emulator_stand.sh`;
как пользоваться — [docs/18](docs/18-emulator-testing.md). Прогон нашёл
полтора десятка дефектов, невидимых тестам, все исправлены (docs/17, «Сессия
15» и «Сессия 16»). Главные: «Подключено» после падения ядра; «Проверить»,
падавшее всегда; лог ядра, не доходивший в релизе; «Назад», закрывавшее
приложение; **старый TUN и спящий поток после каждого переподключения** — дефект
sing-tun, исправлен оверлеем ([ADR-0012](docs/adr/0012-gvisor-reader-stop.md));
латиница в единицах и английский в баннере маршрутизации на русском экране.

Решения владельца («сделай всё сам», 2026-09-23): ADR-0011 принят; для
REALITY ядру всегда отдаётся отпечаток `chrome`; процесс-наблюдатель после
падения ядра **не делаем** (R11, docs/17).

Критерии приёмки M1 2–4 выполнены на эмуляторах (Wi-Fi ↔ LTE на Android 11,
Doze на 16, отказ в VPN-диалоге); **чек-лист утечек прогнан на эмуляторе
целиком** (docs/09, прогоны сессий 15 и 16). Одна находка — платформенная: под
системным lockdown при снятом туннеле **DNS-запросы приложений уходят в сеть
открытым текстом** (их шлёт netd от uid 0, Android его не блокирует).
Соединения при этом блокируются.

**Очередь, по порядку:**

1. **Телефон владельца против его панели** — единственное, чего эмулятор не
...
2. **Решения владельца:** ключ подписи релизов в секреты CI (иначе каждое
   ...
   DNS под lockdown (меняет смысл «Отключить», docs/09 и очередь docs/17).
