...
телефоне — нет.** Выпущено **`v0.1.0-alpha.7`** (2026-09-23; alpha.6 —
часом раньше, без полосы безлимита): один APK и `SHA256SUMS.txt`, описание
релиза — короткое (просьба владельца; прежние несли весь CHANGELOG из-за
`awk`, исправлено). Все релизы и теги до alpha.6 **удалены** владельцем как
нерабочие. Сборка не подписана релизным ключом — поверх прежней не ставится.

Что есть: шесть пакетов, **1443** Dart-теста (из них 111 golden, только на
Linux) и 48 Go-тестов, чистый анализ, ядро sing-box под три ABI **с транспортом
XHTTP**, Android-туннель (`VpnService`, `PlatformInterface` 15/15,
`BootReceiver`, плитка, Doze), все экраны приложения, deep links.

**Последний прогресс:** 2026-09-23, сессия 17 — **туннель на узлах владельца.**
Воспроизведено против его панели (доступ — база знаний, запись 31;
`.env.local`, только чтение): три дефекта, все исправлены.

- REALITY-узлы на **Xray 26.7.28** отвергали клиента: по умолчанию там порог
  версии 26.3.27, а sing-box называет себя 1.8.1
  ([ADR-0013](docs/adr/0013-reality-client-version.md));
- трафик шёл не через выбранный сервер: селектор sing-box предпочитал кэш
  своему `default` ([ADR-0014](docs/adr/0014-selector-default.md));
- «Проверить» и «Замерить все» при поднятом туннеле не получали ни одного
  результата: поток групп открывался до старта ядра и умирал молча.

И запрошенное владельцем: серые недоступные узлы, флаг ЕС у «Авто», поля у
списка, тост после обновления подписки, «Настройки → Сброс» (сеть и
настройки приложения), полная полоса трафика у безлимита. Подробно — docs/17, «Сессия 17».

Сессии 15–16 — эмулятор, стенд, чек-лист утечек (docs/09, docs/18); решения
владельца: ADR-0011 принят, для REALITY — отпечаток `chrome`,
процесс-наблюдатель не делаем (R11).

**Очередь, по порядку:**

1. **Телефон владельца против его панели** — настоящая сотовая сеть, XHTTP за
   CDN, холодный старт (записать число в R11; на эмуляторе ~2,1 с).
