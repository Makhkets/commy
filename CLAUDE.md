...
**Фаза: M0 — фундамент.** Кода приложения ещё нет, есть спецификация.
Актуальный статус и следующий шаг — в [docs/07-roadmap.md](docs/07-roadmap.md).

📐 **Дизайн-система — предложение v1 на утверждении.** Направление «Графит и сигнал»
описано в [docs/04-design-system.md](docs/04-design-system.md), макеты — в
[Figma](https://www.figma.com/design/J416grbo7gykd06gN8cJ8Q) (основы, компоненты,
9 экранов в обеих темах, машина состояний). Пакет `commy_ui` **не создаём**, пока
владелец не утвердит три решения из шапки документа. После утверждения токены
переносятся из Figma один в один — значения там и в документе уже совпадают.

Каркас монорепо, домен, парсеры и Go-ядро от дизайна **не зависят** и делаются
параллельно.
