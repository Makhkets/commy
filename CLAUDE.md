...
Что есть: шесть пакетов, 1296 тестов, из них 101 golden (только на Linux), чистый
...
**Последний прогресс:** 2026-09-21 (сессии 11–12). **Туннель возит трафик** —
проверено против живого VLESS-сервера, а не на мёртвых адресах. Два дефекта,
оба про ответы, которые приложение даёт ядру про устройство:
`registerDefaultNetworkCallback` с поднятым VPN отвечает «дефолтная сеть — это
VPN», и ядро оставалось без интерфейса для дозвона; а `{"type":"local"}` DNS
ядро реализует через `/etc/resolv.conf`, **которого на Android нет**, так что
узел, записанный именем, не дозванивался никогда. Логи стали читаемыми: срезаны
ANSI-цвета, убран дубль каждой строки в debug (ушёл в
`adb logcat -s CommyCore`), у неудачного подключения есть и сервер, и причина.

**Чек-лист утечек (R6) прогнан** — то, что эмулятор может ответить честно:
системный резолвер не работает в обход туннеля, DNS восстанавливается после
отключения, IPv6 уходит в туннель, приложение вне своего туннеля, локальные
адреса напрямую. Результаты — в docs/09 рядом с чек-листом; что осталось на
телефон, названо там же.

Закрыты **#20 (QR)**, **дубликат подписки по URL** (ADR-0008) и **хвост #21**:
`deviceInfo` одиннадцатым методом канала (`x-ver-os`, `x-device-model`),
заглушки `0.0.0.0` как причина, а не сервер, ADR-0009. Уведомление туннеля
больше не пересоздаётся раз в секунду.

**Очередь, по порядку:**

1. **Остаток чек-листа утечек — нужен телефон.** Kill switch при падении ядра,
   ...
   приёмки.
2. **Проверить HWID против настоящей панели владельца.** Заголовки уходят и
   видны на тестовой панели; что с ними делает Remnawave с включённым лимитом
   устройств — знает только владелец.
3. **Холодный старт измерить на телефоне** и записать число в R11.
...
эмуляторе `commy_api36`; остаток чек-листа утечек требует телефона и настоящей
сети; панель с реально включённым лимитом устройств заголовки не видела; у
Kotlin-кода туннеля нет юнит-тестов и негде их держать; golden-тестов у
приложения нет вовсе (вся инфраструктура в `commy_ui`); холодный старт честно
не измерен.
