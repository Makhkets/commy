...
(2026-09-18): на странице релиза один APK на 51 МБ и `SHA256SUMS.txt`. Сессия 13
закоммичена и запушена в `main` (`84339ff`…), в релиз не входит.

Что есть: шесть пакетов, **1370** Dart-тестов (из них 101 golden, только на
...
**Последний прогресс:** 2026-09-21 (сессия 13). **XHTTP работает** — задача
владельца. В sing-box такого транспорта нет; клиент написан нами (`core/xhttp`)
и попадает в закреплённое ядро оверлеем сборки: `go.mod` тот же `v1.13.16`,
...
golden-тестов у приложения нет вовсе; `core/cshared` не существует, хотя
десктопные таргеты `build_core.sh` на него ссылаются.
