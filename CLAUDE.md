...
| R8 | **Не обновляем sing-box «мимоходом».** Бамп версии ядра — отдельный PR со своим прогоном матрицы. Ядро собирается **только** через `scripts/build_core.sh`: он накладывает оверлей (`core/cmd/overlaygen` — XHTTP, правка REALITY и правка sing-tun) и проверяет, что тот попал в библиотеку. Бамп включает перебазирование оверлея — `overlaygen` сам остановит сборку на изменившемся файле или сменившейся версии sing-tun. |
...
│  ├─ cmd/overlaygen/        правки sing-box и sing-tun на время сборки (ADR-0010–0012)
...
**M0 закрыт. M1 написан и проверен на эмуляторах, на телефоне владельца — нет.**
Выпущено `v0.1.0-alpha.4` (2026-09-18). Сессии 13–14 — в `main`; UX-проход
(22.09) и сессия 15 — на ветке `fix/ux-sweep`, в `main` не влиты.

Что есть: шесть пакетов, **1421** Dart-тест (из них 111 golden, только на
...
внутри (бюджет 56 МБ, R11).

**Последний прогресс:** 2026-09-23 (сессия 15) — **лёгкий эмулятор, стенд с
живыми узлами и полный прогон Android.** Эмулятор на этой слабой машине
(2 ядра, интернет ~120 КБ/с): `scripts/emulator.sh`; стенд — девять узлов всех
протоколов, панель как Remnawave с лимитом HWID, зонд трафика:
`scripts/emulator_stand.sh`; как всем пользоваться — [docs/18](docs/18-emulator-testing.md).
Прогон нашёл **тринадцать** дефектов, невидимых тестам, все исправлены
(подробно — docs/17, «Сессия 15»), главные:

- после падения ядра уведомление продолжало говорить «Подключено», а трафик
  шёл мимо (Android оставляет запись сервиса; теперь `START_STICKY` +
  очистка при следующем старте + «Туннель остановился»);
- «Проверить» проваливалось на рабочем туннеле **всегда** (читался чужой
  результат группового url-теста);
- в релизной сборке **лог ядра не доходил вообще** (подписка открывалась до
  старта ядра);
- системное «Назад» закрывало приложение с любого экрана;
- тап «откройте приложение, чтобы подключиться» открывал и не подключал;
  последний интент повторялся после ручного отключения;
- ссылка подписки, которой поделились из бота, не импортировалась;
- объявление панели не показывалось никогда; пинг мерил резолвер.

Критерии приёмки M1 2–4 — выполнены на эмуляторах (Wi-Fi ↔ LTE на Android 11,
Doze на 16, отказ в VPN-диалоге); чек-лист утечек — прогнан, таблица в docs/09.

**Очередь, по порядку:**

1. **Решения владельца:** подтвердить ADR-0011; решить, подменять ли отпечатки
   кроме `chrome` для REALITY (на Xray ≥ 26.9.8 они не работают, чинится
   бампом sing-box); влить `fix/ux-sweep` в `main` и выпустить `alpha.5`.
2. **Телефон владельца против его панели** — единственное, чего эмулятор не
   заменяет: настоящая сотовая сеть, XHTTP за CDN, HWID на настоящей
   Remnawave, холодный старт (записать число в R11; на эмуляторе ~2,1 с).
3. **FakeIP и системный кеш, адресаты E-2/E-3** — последние непрогнанные
   строки чек-листа утечек; на стенде это час работы.
4. **Единицы в русской локали** («6.0 GB», «B/s» → «ГБ», «Б/с»).
...
(гонка в самой платформе, docs/17); отдельный процесс-наблюдатель это бы
закрыл — решение владельца (R11: лишний процесс).
