...
Что есть: шесть пакетов, 657 тестов, чистый анализ, ядро sing-box под три ABI,
...
Чего нет: **живого трафика через туннель**. Ни разу. Подписка владельца отдаёт
заглушку «App not supported» вместо серверов, поэтому четыре критерия приёмки M1
не проверены. Это блокер на стороне панели, не на стороне кода.

**Прежде чем что-то делать, открой
[docs/15-handoff.md](docs/15-handoff.md#очередь-работ-по-убыванию-пользы)** —
там очередь работ с приоритетами, объёмом и точными файлами, и там же
восемнадцать граблей с настоящими причинами. Роадмап отвечает на «что», хэндофф
на «в каком порядке и где».

Перед первой строчкой кода прочитай
