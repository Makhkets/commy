...
Что есть: шесть пакетов, 1198 тестов, из них 99 golden (только на Linux), чистый
...
**Чего не хватает:** очередь работ пуста — закрыты все пункты #1–19
([docs/15-handoff.md § 0](docs/15-handoff.md#очередь-работ-по-убыванию-пользы)),
кроме #20 (QR), который ждёт решения владельца по зависимости для рендера.
Не хватает устройства: ни один экран не видел настоящего трафика, чек-лист
утечек (R6) не пройден, у приложения нет golden-тестов.
...
**Последний прогресс:** 2026-09-17 (сессия 9) — два пункта из списка «что
стоит решить владельцу». Правило «сервер, названный дважды, — одна строка»
сведено из четырёх копий в одну: `NodeDuplicates.folded` в `commy_domain`,
зовут её импорт ссылок, добавление и обновление подписки и
`DriftNodeRepository`. Шестерёнка на Главной больше не дублирует навигацию —
она рисуется только ниже 600 dp, как и просит docs/05; кнопка «+» осталась на
всех ширинах. До этого, в сессии 8, были закрыты **#16** и **#19**:
виджет-тесты на девять непокрытых экранов (приложение 135 → 353) и навигация
для планшета и десктопа. Те тесты сразу нашли десять дефектов «экран
утверждает то, чего конфиг не делает» — список в `CHANGELOG.md` и docs/17.

**Решения владельца получены 2026-09-17.** Осталось написать две вещи, и ни
одна не требует устройства:

1. **Дубликат подписки по одному URL** — решено: **обновлять существующую**.
   Сейчас `AddSubscriptionUseCase` всегда берёт новый id, и та же панель
   заводится второй карточкой. Спецификация и три грабли, которые надо
   закрыть до кода, — docs/17, «Сессия 9».
2. **#20, QR** — решено: **`qr_flutter`**. Хвост #9; `QrPayload` в
   `commy_config` уже строит нужную строку и никем не вызывается.

**Не сделано и важно:** устройства не было ни в одной сессии — ни один экран
не видел настоящего трафика; чек-лист утечек (R6) ждёт телефона с сессии 5;
golden-тестов у приложения нет вовсе (вся инфраструктура в `commy_ui`); версия
в `pubspec.yaml` не бампнута, «Unreleased» копится четвёртую сессию.
`commy_ui` не тронут.
