...
Что есть: шесть пакетов, 730 тестов плюс 99 golden (только на Linux), чистый
...
**Последний прогресс:** 2026-09-10 — закрыты **#6** (переименование и интервал
в меню подписки), **#9** (меню узла по долгому нажатию; QR отложен — нужна
зависимость, §7), **#8** (`SubscriptionScheduler` — автообновление наконец
запускается) и **#7** («Замерить все» пачками по 4, с прогрессом и отменой).
Приложение: 75 тестов вместо 44, `commy_ui` не тронут, golden'ы не менялись.
Следующему: начинать с **#11** (экран DNS), затем #12–13; детали и что не
проверено на устройстве — в docs/17-agent-handoff.md, «Сессия 4».
