...
Что есть: шесть пакетов, 1088 тестов плюс 99 golden (только на Linux), чистый
...
**Последний прогресс:** 2026-09-16 — закрыты **#16** и **#19**, то есть весь
хвост очереди, кроме отложенного по зависимости **#20**. Виджет-тесты написаны
на девять экранов, которых не касался ни один тест; приложение 135 → **353**
теста, монорепозиторий 925 → **1187**. Планшет и десктоп получили навигацию:
`AppSection` — четыре раздела, объявленные один раз и разданные всем девяти
экранам, Главная делится на две панели там, где они помещаются. Ниже 600 dp
дерево виджетов то же, что было.

**Тесты сразу окупились:** нашли десять дефектов «экран утверждает то, чего
конфиг не делает», все закрыты здесь же. Крупнейшие: подписка, где панель
отдала один сервер дважды, **не обновлялась вообще** (UNIQUE constraint);
финальное правило врало в режиме Direct (R6); правила вне режима «Правила»
молча не применялись; импорт считал разобранное, а не сохранённое. Список
целиком — в `CHANGELOG.md` и docs/17, «Сессия 8».

**Не сделано и важно:** ни одна строка этой сессии не видела устройства —
полоса 600–1000 dp проверена только виджет-тестами; чек-лист утечек (R6) ждёт
телефона с сессии 5; golden-тестов у приложения нет вовсе (вся инфраструктура
в `commy_ui`); версия в `pubspec.yaml` не бампнута, «Unreleased» копится третью
сессию. Работа **не закоммичена** (R10). `commy_ui` не тронут.
