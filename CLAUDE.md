...
| R8 | **Не обновляем sing-box «мимоходом».** Бамп версии ядра — отдельный PR со своим прогоном матрицы. Ядро собирается **только** через `scripts/build_core.sh`: он накладывает оверлей (`core/cmd/overlaygen` — XHTTP и правка REALITY) и проверяет, что тот попал в библиотеку. Бамп включает перебазирование оверлея — `overlaygen` сам остановит сборку на изменившемся файле. |
...
│  ├─ cmd/overlaygen/        правки sing-box на время сборки (ADR-0010, ADR-0011)
...
(2026-09-18): на странице релиза один APK на 51 МБ и `SHA256SUMS.txt`. Сессии 13
и 14 закоммичены и запушены в `main`, в релиз не входят.

Что есть: шесть пакетов, **1392** Dart-теста (из них 111 golden, только на
Linux) и 35 Go-тестов, чистый анализ, ядро sing-box под три ABI **с транспортом
XHTTP**, Android-туннель (`VpnService`, `PlatformInterface` 15/15,
`BootReceiver`, плитка, Doze), все экраны приложения, deep links, APK с ядром
внутри (51,3 МБ из 56).

**Последний прогресс:** 2026-09-21 (сессия 14), два шага роадмапа.

1. **Блокировщик рекламы работает, и работает в DNS.** Переключатель стоял с
   самого начала и не мог сработать никогда: он просил набор `geosite-ads`, а
   такого файла не публикует ни одно зеркало. Теперь список —
   `geosite-category-ads-all`, тот самый файл источника E-2, и запрос к
   рекламному имени получает `REFUSED` у резолвера, а не адрес, по которому
   потом отказывают в соединении (правило маршрутизации осталось: имя может
   прийти из системного кеша). Проверено на закреплённом ядре с настоящим
   списком — три документа, таблица в docs/17, «Сессия 14». Нового в сеть не
   добавилось. **Чек-лист утечек по этому изменению не пройден: нужен телефон.**
2. **У приложения появились golden-тесты** — десять картинок, пять экранов ×
   две темы, через настоящие провайдеры. `melos run test:golden` и CI-джоба
   захватывают теперь и приложение. Заодно починен `capture_screenshots.dart`,
   который висел после каждого снимка (`toImage` ждали внутри fake-async зоны).

**Сессия 13** — **XHTTP работает**, задача владельца. В sing-box такого
транспорта нет; клиент написан нами (`core/xhttp`) и попадает в закреплённое
ядро оверлеем сборки: `go.mod` тот же `v1.13.16`,
новых модулей ноль, +177 КБ к AAR ([ADR-0010](docs/adr/0010-xhttp-transport.md)).
Проверено против **настоящего Xray** (релиз 26.3.27 и 26.9.9): все режимы ×
HTTP/1.1, HTTP/2, HTTP/3 и REALITY, 24 МиБ вниз и 16 МиБ вверх со сверкой
хэшей, и в эмуляторе — импорт, подключение, переключение на живом туннеле.
Повторить: `XRAY_BIN=… scripts/xhttp_interop.sh`.

**Найдено по дороге, и это важнее самого XHTTP:** обычный **REALITY перестаёт
работать против Xray ≥ 26.9.8** — sing-box вырезает из ClientHello key share
`X25519MLKEM768`, а новый сервер без него клиента отвергает. Ни один выпущенный
sing-box не проходит. Исправлено третьей правкой оверлея (современный hello
первым, обрезанный — после отказа сервера), проверено на восьми поколениях Xray
от 1.8.4 до 26.9.9. **[ADR-0011](docs/adr/0011-reality-client-hello.md) ждёт
подтверждения владельца**: правка меняет то, как ядро выглядит в сети.

Ещё три дефекта закрыты: один «кривой» узел больше не роняет сборку документа
для всех серверов; на узел, появившийся при поднятом туннеле, можно
переключиться; uTLS-отпечаток не уходит в QUIC-протоколы.

**Очередь, по порядку:**

1. **Решения владельца:** подтвердить ADR-0011; решить, подменять ли отпечатки
   кроме `chrome` для REALITY — на Xray ≥ 26.9.8 они не работают и после
   правки (в этом uTLS гибридный share есть только у Chrome), а чинится это
   только бампом sing-box.
2. **XHTTP на настоящем узле владельца** (Remnawave, за CDN, если есть) —
   локальный стенд этого не заменит.
3. **Остаток чек-листа утечек — нужен телефон.** Kill switch при падении ядра,
   переход Wi-Fi ↔ мобильная, выход из сна, перезагрузка с автозапуском,
   `strict_route`. Это единственное, что мешает закрыть M1 по критериям
   приёмки. Сюда же — правило блокировщика из сессии 14: оно меняет секцию
   `dns`, а значит по docs/09 требует того же чек-листа.
4. **Проверить HWID против настоящей панели владельца.**
5. **Холодный старт измерить на телефоне** и записать число в R11.
...
**Не сделано и важно:** настоящего телефона не было ни разу — всё проверено в
эмуляторе `commy_api36`; `downloadSettings` у XHTTP не реализован (узел
импортируется, оба направления идут основным маршрутом); Apple-сборка с
оверлеем не запускалась (нет macOS); у Kotlin-кода туннеля нет юнит-тестов;
автообновление списков блокировки по интервалу (E-3 его разрешает) не сделано —
только загрузка по нажатию; `core/cshared` не существует, хотя десктопные
таргеты `build_core.sh` на него ссылаются.

**Живой туннель:** панель не нужна. XHTTP и REALITY — `scripts/xhttp_interop.sh`
(docs/17, «Сессия 13»); обычный узел на этой же машине за пять минут — docs/17,
«Сессия 11»; сборка в эмуляторе — docs/17, «Сессия 10». Против живой подписки —
[docs/16-local-testing.md](docs/16-local-testing.md) (`.env.local` на этой
машине нет).
