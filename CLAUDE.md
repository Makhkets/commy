...
телефоне — нет.** Выпущено **`v0.1.0-alpha.6`** (2026-09-23): один APK и
`SHA256SUMS.txt`, описание релиза — короткое (просьба владельца). Все прежние
релизы и теги (alpha.1–5) **удалены** владельцем как нерабочие. Сборка не
подписана релизным ключом — поверх прежней не ставится.

Что есть: шесть пакетов, **1440** Dart-тестов (из них 111 golden, только на
...
настройки приложения). Подробно — docs/17, «Сессия 17».
