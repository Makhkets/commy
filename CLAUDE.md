...
Что есть: шесть пакетов, 826 тестов плюс 99 golden (только на Linux), чистый
...
**Последний прогресс:** 2026-09-14 — закрыты **#15** и **#18**. Над списком
серверов (начиная с двух) — поиск по имени/адресу/коду страны и чип порядка:
как в панели, по задержке, по имени (`NodeSort` в домене, `AppSettings.nodeSort`).
Статистика хранит неделю по дням: `TrafficDay` и порт `TrafficHistoryRepository`
в домене, `trafficHistoryPumpProvider` с корня пишет дельты и сбрасывает базу на
отключении, экран показывает неделю и без туннеля. Ручные группы оставлены в
схеме без UI — создать их негде.
Приложение: 135 тестов, всего 826 + 99 golden; `commy_ui` не тронут.
На машине владельца поставлен полный Linux-тулчейн (docs/15 § 5 «Linux»);
сборки — в `~/Work/commy-builds/`. **Не сделано и важно:** ни одна из двух задач
не видела телефона; чек-лист утечек (R6) по-прежнему ждёт устройства; версия
в `pubspec.yaml` не бампнута, CHANGELOG «Unreleased» копится. Следующему:
хвост очереди — **#16**, **#19**, **#20**; детали — в docs/17-agent-handoff.md,
«Сессия 7».
